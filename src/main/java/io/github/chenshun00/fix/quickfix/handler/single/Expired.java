 ...
 */
@Component
public class Expired extends FixHandler {

    //回复一个已报, 一个过期
    @Override
    public void handleMessage(Message src, SessionID sessionID) throws FieldNotFound {
        this.handleNew(src, sessionID);
        this.handleExpired(src, sessionID);
    }

    public void handleExpired(Message src, SessionID sessionID) throws FieldNotFound {
        ...
        res.setField(new ExecType(ExecType.EXPIRED));
        res.setField(new OrdStatus(OrdStatus.EXPIRED));
